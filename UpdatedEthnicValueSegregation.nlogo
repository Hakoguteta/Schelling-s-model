turtles-own [
  ...
  other-nearby
  value-orientation
  preference-weight     ; weight between ethnic and value-based preferences
...
to setup
  ...
  ask patches [
    ...
    if random 100 < density [
      sprout 1 [
        set color ifelse-value (random 100 < fraction_majority) [105] [27]  ;; Ethnic division

        ;; Handle the different versions (Extension and Original_Schelling)
        if Version = "Extension" [
          ;; Assign turtles to be either value-oriented or ethnic-oriented based on fraction_value_oriented
          ifelse random 100 < fraction_value_oriented [
            ;; Value-oriented turtles (circle)
            set preference-weight 1.0
            set shape "circle"
          ] [
            ;; Ethnic-oriented turtles (square)
            set preference-weight 0.0
            set shape "square"
          ]
        ]

        if Version = "Original_Schelling" [
          ;; In the Original Schelling model, all turtles are ethnic-oriented (square)
          set preference-weight 0.0
          set shape "square"
        ]

        ;; Set a random initial value orientation for value-oriented turtles (even for the Original Schelling version)
        set value-orientation random-float 1
      ]
    ]
  ]
  update-turtles
